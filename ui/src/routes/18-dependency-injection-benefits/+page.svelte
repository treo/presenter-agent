<script>
	import { ImageBlob } from '$lib/components';
</script>

<div class="slide">
	<h1 class="slide-title">Dependency Injection: Benefits</h1>
	<div class="grid grid-cols-[2fr_1fr] flex-grow gap-6">
		<div class="flex flex-col">
			<ul class="bullet-list">
				<li>Easily replace implementations</li>
				<li>Changing requirements</li>
				<li>Different environments</li>
				<li>Testing</li>
				<li>Enforces Modularity</li>
				<li>Developing for common interfaces</li>
			</ul>
			<div class="flex-grow"></div>
		</div>
		<div>
			<ImageBlob
				imageName="images-053.png"
				borderRadius="30% 70% 50% 50% / 60% 40% 25% 75%"
				bgPosition="bg-right"
			/>
		</div>
	</div>
</div>
